...
var watchr  	= require('watchr')
  ...
  , g15composer	= require('g15composer')
  , growl		= require('growl')
...
commander
  ...
  .option('-n, --notifier [type]', 'Use a notification service [growl]', 'growl')
  .parse(process.argv);
...
function event_processor(arg, filename, directory, ext) {
	switch(arg[0]) {
		case "create":
			...
			notify('A file has been created', directory+config.os_dir_oblique+filename+' ('+arg[2].size+'bytes)')
			switch(ext) {
				default:
		...
		case "unlink":
			...
			console.log(ascii.color('  EXTENSION  : ', 'cyan') + ext);
			console.log(ascii.color('  * REFRESHING ALL PAGES *', 'bold'));
			notify('A file has been deleted', directory+config.os_dir_oblique+filename)
			io.sockets.emit('update', { type: 'refresh' });
		break;
		case "update":
			//change File
			...
			notify('A file has been changed', directory+config.os_dir_oblique+filename+' ('+arg[2].size+'bytes)')
			switch(ext) {
				default:
					console.log(ascii.color('  * REFRESHING ALL PAGES *', 'bold'));
					notify('Refreshing Displays!', filename+' has changed, refreshing connected displays')
	...
	new compressor.minify({
		...
		callback: function(err){
		    if(err) { 
		    	...
		    	console.log(ascii.color('  * RELOADING CSS ENTRIES *', 'bold'));
				notify('Successfuly compiled CSS', filename+' > '+file_out)
		    	if(!config.cssreload) {io.sockets.emit('update', { type: 'refresh' }) } else { io.sockets.emit('update', { type: 'css' }); }
		    }
		}
	});
	...
	new compressor.minify({
		...
		callback: function(err){
			if(err) { 
				...
				console.log(ascii.color('  * REFRESHING ALL PAGES *', 'bold'));
				notify('Successfuly compiled JS', filename+' > '+file_out)
				io.sockets.emit('update', { type: 'refresh' });
			}
		}
	});
